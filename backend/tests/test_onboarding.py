"""Tests for onboarding fields on UserRecord."""
...
def _patch_beanie_user():
    """Bypass Beanie collection init so UserRecord can be constructed without MongoDB."""
    mock_settings = MagicMock()
    mock_settings.pymongo_collection = MagicMock()
    with patch.object(UserRecord, "get_settings", return_value=mock_settings):
        yield
...
class TestUserRecordOnboardingDefaults:
    ...
    def test_onboarding_skipped_steps_not_shared_across_instances(self):
        user_a = UserRecord(clerk_user_id="test_a")
        user_b = UserRecord(clerk_user_id="test_b")
