/** E2E Demo: Onboarding guided tour — side panel with 5 steps. */
...
test.describe('Onboarding Guided Tour', () => {
  ...
  test('wizard panel appears and completes full flow', async ({ page }) => {
    // Navigate to app — should show transactions with onboarding panel
    ...
    await expect(panel.locator('.ob-step-title')).toHaveText('Welcome to Comparari');
    ...
    await expect(panel.locator('.ob-step-heading')).toHaveText('Start Your First Transaction');
    ...
    await expect(summaryItems).toHaveCount(3); // profile, documents, extraction

    // Should show next steps cards
    await expect(panel.locator('.ob-next-steps-card')).toHaveCount(2); // integrations + transactions

    // Click "Go to Transactions" to finish
    await panel.getByRole('button', { name: 'Go to Transactions' }).click();
    await pace();
